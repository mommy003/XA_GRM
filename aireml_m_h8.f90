...
subroutine aireml_m_h (mbin,yidx,yv,rn,rnm,trn,pedn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl11rr,fl11rrr,fl24,xmm,nit,conv,wt_res)        
...
if (fl11.ne.'null') then
  ...
  !inquire(file=trim(fl11)//".dsgn",exist=file_exist)
  !if (file_exist) then
  !  open (unit=52,file=trim(fl11)//".dsgn",status='old')
  !  do i=1,rn
  !    read(52,*)cdum,cdum,(yidx2(1:mn,yi,i),yi=1,tn)
  !  end do
  ...
  !          trny=trny+rnm(yi)
  !          do j=1,rnm(yi)
  !            v1=v1+mbin(k,yidx2(k,yi,j),zi)*vcva(k,xi,yi)*py(trny-rnm(yi)+j,1)
  !            if (yidx2(k,yi,j)==zi) then
...
if (fl11r.ne.'null') then
  ...
  !inquire(file=trim(fl11r)//".dsgn",exist=file_exist)
  !if (file_exist) then
  !  open (unit=52,file=trim(fl11r)//".dsgn",status='old')
  !  yidx2=0
  !  do i=1,rn
  !    read(52,*)cdum,cdum,(yidx2(1:mn,yi,i),yi=1,tn)
  !    print*,yidx2(:,1,i),"     ",yidx(1,i)
  !  end do
  ...
  !          trny=trny+rnm(yi)
  !          do j=1,rnm(yi)
  !            if (yidx2(k,yi,j).ne.0) then
  !              !v1=v1+mbin(k,yidx2(k,yi,j),zi)*vcva(k,xi,yi)*py(trny-rnm(yi)+j,1)
  !              v1=v1+mbin(1,yidx2(k,yi,j),zi)*vcva(k,xi,yi)*py(trny-rnm(yi)+j,1)
  !              if (yidx2(k,yi,j)==zi) then
  !                v2=v2+vcva(k,xi,yi)*py(trny-rnm(yi)+j,1)
  !              end if
  !            end if
  ...
  !              do j=1,rnm(yi)
  !                if (yidx2(k,yi,j).ne.0) then
  !                  !v1=v1+mbin(k,yidx2(k,yi,j),zi)*vcva(k,vi,yi)*pm(trny-rnm(yi)+j,trnx-rnm(xi)+i)
  !                  v1=v1+mbin(1,yidx2(k,yi,j),zi)*vcva(k,vi,yi)*pm(trny-rnm(yi)+j,trnx-rnm(xi)+i)
  !                end if
  !              end do
  !            end do
  !            if (yidx2(k,xi,i).ne.0) then
  !              !v2=v2+v1*mbin(k,yidx2(k,xi,i),zi)
  !              !v2=v2+v1*mbin(1,yidx2(k,xi,i),zi)
  !              v2=v2+v1*mbin(1,zi,yidx2(k,xi,i))
  !            end if
  !          end do
  !        end do
  !        blup_r(zi,vi,k)=v2
