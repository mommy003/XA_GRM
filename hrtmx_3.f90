subroutine hrtmx_ (hrtmx,grm,pedn,n,mn,fam,tune,blend,method,f41,filnam,f42)
...
character(len=64)::hrtmx,f41,filnam,f42
  ...
  do i=1,pedn
    if (ped(zi,3) == ped(i,2)) then
      if (i >= zi) then
        ...
        print*,'offspring (',trim(ped(zi,2)),') is before parents (',trim(ped(zi,3)),')'
        pause
      end if
      goto 10 
    end if
...
10 continue
  ...
  do i=1,pedn
    if (ped(zi,4) == ped(i,2)) then 
      if (i >= zi) then
        !print*,'check >>> offspring is before parents'
        print*,'offspring (',trim(ped(zi,2)),') is before parents (',trim(ped(zi,4)),')'
        pause
      end if
      goto 20 
    end if
...
print*,'pedigree indexed for A matrix *****************************'
...
do zi=1,pedn
  ...
  print*,'something wrong >>> check, pedigree #',zi
  pause 
30 continue
...
!!$OMP END PARALLEL DO
print*,'indexed pedigree checked **********************************'
