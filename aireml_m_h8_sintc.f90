...
subroutine aireml_m_h_dsgn3 (mbin,yidx,yv,rn,rnm,trn,pedn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,fl_dsgn,cov_idx)        
...
integer::dsgn(rn,1),dsgn2(tn,rn,1)
real::dsgnd(rn,1),dsgn2d(tn,rn,1),wjr,a1,a2,a3
...
!desing matrix reading and make Z2AZ2', (Z1AZ2' + Z2AZ1')
  ...
  do i=1,rn  !same as pedn, order follows fam file
    ...
    do j=1,rn
      if (i.ne.j) then
        !zm2(i,j)=1/(dsgnd(i,1)-dsgnd(j,1))**2
        zm2(dsgn(i,1),dsgn(j,1))=1/(dsgnd(i,1)-dsgnd(j,1))**2
        !x1=x1+zm2(i,j)
        x1=x1+zm2(dsgn(i,1),dsgn(j,1))
        x2=x2+1
        !x3=x3+zm2(i,j)**2
        x3=x3+zm2(dsgn(i,1),dsgn(j,1))**2
        !print*,zm2(i,j),a1,a2,a3
      end if
    end do
    do j=1,rn
      if (i.ne.j) then
        !!zm2(i,j)=(zm2(i,j)-(x1/x2))/(((x3-(x1**2/x2))/x2)**0.5*x2**0.5)
        !zm2(dsgn(i,1),dsgn(j,1))=(zm2(dsgn(i,1),dsgn(j,1))-(x1/x2))/(((x3-(x1**2/x2))/x2)**0.5*x2**0.5)
        !!print*,zm2(i,j),a1/a2,a3,a1**2,a2**0.5,(a3-(a1**2))/a2
        !!pause
      end if
    end do
...
if (fl11.ne.'null') then
  ...
  do xi=1,tn
            ...
            do j=1,rnm(yi)
              if (k==1) then
                ...
                do i=1,rn
                  ...
                  v3=v3+mbin(1,zi,dsgn(i,1))*vcva(3,xi,yi)*zm2(dsgn2(yi,j,1),dsgn(i,1))
                end do
              ...
              elseif (k==2) then
                ...
                do i=1,rn
                  ...
                  v3=v3+mbin(1,zi,dsgn(i,1))*vcva(k,xi,yi)*zm2(dsgn2(yi,j,1),dsgn(i,1))
                end do
                v1=v1+v3*py(trny-rnm(yi)+j,1)
        ...
        do yi=1,tn
          ...
          do j=1,rnm(yi)
            ...
            !if (zm2(zi,dsgn2(yi,j,1)).ne.0) then
              ...
              !v1=v1+vcva(4,xi,yi)*zm2(dsgn(zi,1),dsgn2(yi,j,1))*py(trny-rnm(yi)+j,1)
              v1=v1+vcva(4,xi,yi)*zm2(dsgn2(yi,j,1),dsgn(zi,1))*py(trny-rnm(yi)+j,1)
              !print*,zm2(dsgn(zi,1),dsgn2(yi,j,1)),dsgn(zi,1),dsgn2(yi,j,1),j
              !pause
            !end if
    ...
    !blup_mee(1:pedn,1)=matmul(transpose(zm2*vcva(4,1,1)),py)
    !tyv=matmul(transpose(zm2),py)
    !print*,tyv
    !print*,''
            ...
            do i=1,rnm(xi)
                ...
                do j=1,rnm(yi)
                  ...
                  elseif (k==2) then
                    ...
                    do ix=1,rn
                      ...
                      v3=v3+mbin(1,zi,dsgn(ix,1))*vcva(2,vi,yi)*zm2(dsgn2(yi,j,1),dsgn(ix,1))
                    end do
              ...
              if (k==1) then
                ...
                do ix=1,rn
                  ...
                  v3=v3+mbin(1,zi,dsgn(ix,1))*vcva(3,vi,xi)*zm2(dsgn2(xi,i,1),dsgn(ix,1))
                end do
                v22=v22+v11*v3
                v222=v222+2*(v1*v3)
                !print*,v22,v222
              elseif (k==2) then
                ...
                do ix=1,rn
                  ...
                  v3=v3+mbin(1,zi,dsgn(ix,1))*vcva(2,vi,xi)*zm2(dsgn2(xi,i,1),dsgn(ix,1))
                end do
                v2=v2+v1*v3
    ...
    do xi=1,tn
      ...
      do zi=1,rn    !=pedn
        ...
        do yi=1,tn
          trny=trny+rnm(yi)
          do j=1,rnm(yi)
              !v1=v1+vcva(4,xi,yi)*zm2(zi,dsgn2(yi,j,1))*py(trny-rnm(yi)+j,1)
              v1=v1+vcva(4,xi,yi)*zm2(dsgn2(yi,j,1),dsgn(zi,1))*py(trny-rnm(yi)+j,1)
          end do
        end do
        blup_mee(zi,xi)=v1
      ...
      do xi=1,tn
        ...
        do i=1,rnm(xi)
          ...
          do yi=1,tn
            trny=trny+rnm(yi)
            do j=1,rnm(yi)
                !v1=v1+vcva(4,vi,yi)*zm2(zi,dsgn2(yi,j,1))*pm(trny-rnm(yi)+j,trnx-rnm(xi)+i)
                v1=v1+vcva(4,vi,yi)*zm2(dsgn2(yi,j,1),dsgn(zi,1))*pm(trny-rnm(yi)+j,trnx-rnm(xi)+i)
            end do
          end do
            !v2=v2+v1*zm2(zi,dsgn2(xi,i,1))*vcva(4,vi,xi)
            !v2=v2+v1*zm2(dsgn(zi,1),dsgn2(xi,i,1))*vcva(4,vi,xi)
            v2=v2+v1*zm2(dsgn2(xi,i,1),dsgn(zi,1))*vcva(4,vi,xi)
        end do
      end do
      !blup_mee_r(zi,vi)=v2
