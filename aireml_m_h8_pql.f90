...
subroutine aireml_m_h_pql (mbin,yidx,yv,rn,rnm,trn,pedn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res)        

!***********************************************************************
!aireml_b: multivariate analysis especially for case control 
!including penalised quasi likelihood (PQL)
!S. Hong Lee (2020)
...
INTEGER::n,nped,ix,iz,tn,nb,nbc,nbc2,xi,yi,ri,zi,zj,vi,ui,wi,wj,nit
integer::rn,pedn,mn,trn,trnx,trny,tfn,trnv,trnu,itit,itit2
...
double precision::resw_rsv(trn),tyv_rsv(trn,1),tyv2(trn,1),pv,vv
...
do i=1,trn
  v1=0
  do j=1,tfn
    v1=v1+beta(j,1)*xm(i,j)
  end do
  pv=1/(1+exp(-v1-blup_ebv(i,1,1)))
  vv=pv*(1-pv)
  tyv(i,1)=v1+blup_ebv(i,1,1)+(tyv2(i,1)-1.0D0-pv)/vv
  !tyv(i,1)=v1+blup_ebv(i,1,1)+(int(tyv2(i,1)-1)-pv)/vv
  resw(i)=1/vv
  !print*,tyv(i,1),v1,blup_ebv(i,1,1),pv,vv
end do
  ...
  do xi=1,tn
    ...
    !vcve(xi,xi)=1 !((x2-(x1**2/rnm(xi)))/(rnm(xi)-1))/2
...
110 continue
  ...
  !if (itit==1.and.itit2==1) then
      ...
      do zj=1,pedn
        do k=1,mn
          ...
          do yi=1,tn
            trny=trny+rnm(yi)
            do j=1,rnm(yi)
              v1=v1+mbin(k,yidx(yi,j),zj)*vcva(k,xi,yi)*py(trny-rnm(yi)+j,1)
            end do
          end do
          blup_ebv(zj,xi,k)=v1
        end do !k
    ...
    !see page 654 in AJHG (2016) 98: 653. 
    do i=1,trn
      v1=0
      do j=1,tfn
        v1=v1+beta(j,1)*xm(i,j)
      ...
      !tyv(i,1)=v1+blup_ebv(i,1,1)+(int(tyv2(i,1)-1)-pv)/vv
      resw(i)=1/vv
      !print*,tyv(i,1),v1,blup_ebv(i,1,1),pv,vv,resw(i),tyv2(i,1)
    end do
    !print*,beta
    !pause
  end if
    ...
    if ((LKH>-99999999 .and. LKH<99999999) ) then
      ...
      if (zi==1) then     !the first iteration
        ...
        do xi=1,tn
          ...
          !vcve(xi,xi)=1 !((x2-(x1**2/rnm(xi)))/(rnm(xi)-1))*(1-sv_fact)
...
111 continue
    ...
    do xi=1,tn
      ...
      !vcve(xi,xi)=1 !vcve(xi,xi)+up(xi,1)
    end do
    do wi=1,mn
      do xi=1,tn
...
1000 continue 
  ...
  !imputed phenotypes record
  open (unit=48,file=trim(fl5)//".impy",status='unknown')
  do i=1,trn
    write(48,*)i,tyv(i,1)
  end do
  close(48)
