...
subroutine aireml_rxe_ML_NR (yidx,yv,rn,rnm,trn,pedn,ttn,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl12,fl24,xmm,nit,conv,thn)        
...
double precision::xvmx(tfn,tfn),xvm(tfn,trn),xvmx2(tfn,tfn)
  ...
  !pause
   ...
   !print*,'beta:',beta
    ...
    do i=1,trn
      ...
      !v2=v2+py(i,1)*tyv(i,1)   !REML
      v2=v2+res(i,1)**2*vmi(i)   !ML
      !llik(i)=-0.5*(log(1/vmi(i))+py(i,1)*tyv(i,1)) !REML, ignoring ln(XV-1X)
      llik(i)=-0.5*(log(1/vmi(i))+(res(i,1)*res(i,1))*vmi(i))     !maximum likelihood
    end do
    !print*,py(1:10,1)

    !LKH=-0.5*(LA+LG+v2)
    !LKH=-0.5*(trn*log(2*3.14159)+LA+v2)
    LKH=-0.5*(LA+v2)
    ...
    do xi=1,tn
      ...
      do i=1,trn    !same as pedn
        !v2=v2+vmi(i)*vmi(i)                     !Fisher
        v2=v2+0.5*vmi(i)*vmi(i)-(res(i,1)*vmi(i))**2*vmi(i)   !NR 
      end do
      aim(xi,xi)=v2
      !print*,v2
      ...
      do i=1,trn
        ...
        dldv_sc(i,xi)=0.5*((res(i,1)*vmi(i))**2-vmi(i))
      end do 
      dldv(xi,1)=sum(dldv_sc(:,xi))    !v2*(0.5)+tr1
      !print*,'dldv ve:',dldv(xi,1)
    end do !xi
...
50        continue
          ...
          v3=0
          do i=1,trn    !same as pedn
            !v3=v3+(vmi(i)*pig_vcva(1,i))**2
            v3=v3+0.5*(vmi(i)*pig_vcva(1,i))**2-(res(i,1)*vmi(i)*pig_vcva(1,i))**2*vmi(i)
          end do
          ...
          do vi=1,tn
            v2=0
            do i=1,trn
              !v2=v2+(vmi(i)*pig_vcva(1,i)*vmi(i))
              v2=v2+0.5*(vmi(i)*pig_vcva(1,i)*vmi(i))-(res(i,1)*vmi(i))**2*pig_vcva(1,i)*vmi(i)
              !print*,trn,i,v2
            end do
            aim(idum+xi,vi)=v2
            aim(vi,idum+xi)=v2
            !print*,'vg x ve:',idum+xi, v2
          end do !vi
          ...
          do vi=1,xi-1
            ...
            do i=1,trn !rnm(vi)
              ...
              !v2=v2+vmi(i)*pig_vcva(k,i)*vmi(i)*pig(i)   !V-1 A V-1 D 
              v2=v2+0.5*vmi(i)*pig_vcva(k,i)*vmi(i)*pig(i)-(res(i,1)*vmi(i))**2*pig_vcva(k,i)*pig(i)*vmi(i)   !V-1 A V-1 D 
            end do
            !aim(idum+xi,idum+vi)=v2
            aim(idum+vi,idum+xi)=v2
            !print*,'vg x vg:',v2
          end do
          ...
          tr1=0
          do i=1,trn
            dldv_sc(i,idum+xi)=0.5*((res(i,1)*vmi(i))**2*pig(i)-vmi(i)*pig(i))
          end do
          dldv(idum+xi,1)=sum(dldv_sc(:,idum+xi)) !************************************
          !print*,'dldv vg:',dldv(idum+xi,1)
        end do !xi
...
53          continue
            ...
            v2=0
            do i=1,trn
              !v2=v2+(vmi(i)*pig_vcva(1,i))**2
              v2=v2+0.5*(vmi(i)*pig_vcva(1,i))**2-(res(i,1)*vmi(i)*pig_vcva(1,i))**2*vmi(i)
            end do
            aim(idum+m_tnk(ii,k)+nbc,idum+m_tnk(ii,k)+nbc)=v2     !check
            !print*,'cov***:',v2

            !for cov x Ve (off diagonal) ************************************
            v2=0
            do i=1,trn
              !v2=v2+vmi(i)*pig(i)*vmi(i) 
              v2=v2+0.5*vmi(i)*pig(i)*vmi(i)-(res(i,1)*vmi(i))**2*pig(i)*vmi(i)
              !print*,i,trn,v2,pig(i)
            end do
            aim(idum+m_tnk(ii,k)+nbc,vi)=v2          !check with Ve1
            aim(vi,idum+m_tnk(ii,k)+nbc)=v2          !check with Ve1
            !print*,'cov x ve:',v2
            ...
            do vi=1,m_tnk(ii,k)
              ...
              do i=1,trn
                !eig simplification  >>> check when using multiple GRMs
                !v2=v2+vmi(i)*pig(i)*vmi(i)*pig_vcva(k,i)
                v2=v2+0.5*vmi(i)*pig(i)*vmi(i)*pig_vcva(k,i)-(res(i,1)*vmi(i))**2*pig(i)*pig_vcva(k,i)*vmi(i)
              end do
              aim(idum+m_tnk(ii,k)+nbc,idum+vi)=v2  !with the same Vg1 for first trait
              aim(idum+vi,idum+m_tnk(ii,k)+nbc)=v2  !with the same Vg1 for first trait
              !print*,'cov x vg:',v2
            end do

          ! dldv for cov
          do i=1,trn
            dldv_sc(i,idum+m_tnk(ii,k)+nbc)=0.5*((res(i,1)*vmi(i))**2*pig(i)-vmi(i)*pig(i))
          end do
          dldv(idum+m_tnk(ii,k)+nbc,1)=sum(dldv_sc(:,idum+m_tnk(ii,k)+nbc))   !v1*0.5+tr1
          !print*,'dldv cov:',dldv(idum+m_tnk(ii,k)+nbc,1)
        end do !yi
      end do !xi
...
96          continue
  ...
  open (unit=55,file=trim(fl5)//".vuong_2.1",status='unknown')
    !write(55,'(100g18.8)')xvmx2(1,1),0,0,0         !for fixed effects
    !write(55,'(100g18.8)')0,infm(1,1),infm(1,3:4)
    !write(55,'(100g18.8)')0,infm(3,1),infm(3,3:4)
    !write(55,'(100g18.8)')0,infm(4,1),infm(4,3:4)
  ...
  do i=1,rn
    !write(56,'(100g18.8)')xm(i,1)*res(i,1),dldv_sc(i,1),dldv_sc(i,3:4)
