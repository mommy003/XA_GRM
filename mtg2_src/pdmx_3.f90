...
allocate(rtmx(nid,nid),rtmx2(nid,nid),rid(ncol))
...
if (mn==0) then
  ...
  !$OMP PARALLEL DO PRIVATE(i,j,k,v1,i1)
  do i=1,nid
    !do j=1,nid
    do j=1,i
      v1=0;i1=0
      do k=1,ncol
        if (kvm(i,k).ne.-99999.and.kvm(j,k).ne.-99999) then
          i1=i1+1
...
do i=1,nid
  do j=1,i 
    if (rtmx(i,j).ne.0.0D0) then
      write(41,'(i8,i8,f24.8,i8)')i,j,rtmx(i,j),rtmx2(i,j)
    end if
  end do
end do
close(41)

deallocate(rtmx,rtmx2)
