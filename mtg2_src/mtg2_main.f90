...
use fileio
implicit none
integer::n,pedn,rn,xi,i,j,k,k2,kk,jj,tn,zi,tfn,trn,thn,pca,bend
integer::inv,chol,mrrm,mrnm,rxe,wmj,cov_idx,mtpreg !,wmi
...
real::r1,rtmx,blend,sclf
!double precision::r1

integer::io,ioerr,mn,narg,mg,bmg,zmg,tmg,cc,qc,cc_c,qc_c,matmat,t2b,pql,tune
integer,allocatable::idx_ped(:,:),idx_obs(:,:),yidx(:,:)
integer,allocatable::idx_in(:),inlst(:),outlst(:)

double precision::irgt
integer::icove,rrme,nit,frq,aug,OMP_GET_NUM_THREADS,gwas,check,method
character(len=64)::fl1,fl2,filnam,fl3,fl4,fl24,fl5,fl6,fl7,hrtmx,nrm,wt_par,wt_res
character(len=64)::rtmx2,rtmx3,prs,targ
character(len=64)::fl8,fl9,fl10,fl11,fl11r,fl12,fl_rrm,fl_spl,plink,vgpy,sbv
character(len=64)::sbv2,ginv,fl11rr,fl11rrr,grmout,subgrm
character(len=64)::fl_pred,fl_rnm,fl_mcv,matvec,fl111,fl111r,fl_dsgn,fl_dsgn2,fl_dsgn3
...
print*,"******************************************************************"
print*,"MTG2 version 2.22 (Oct2021)"
print*,"******************************************************************"
...
fl11='null'
fl11r='null'
fl11rr='null'      !P matrix output for aireml_m_h and aireml_m_h2
fl11rrr='null'     !GPG matrix output (G=A*var(g)) 
fl111='null'
fl111r='null'
fl12='null'
fl4='null'
...
mtpreg=0
cc=0;qc=0
tn=1         !default: 1 traits 
icove=0       !defoult: no cove
check=1       !defoult: checking pedigree, data duplication, missing
...
fl_dsgn='null'       !design matrix for maternal genetic effects
fl_dsgn2='null'       !design matrix for social interaction effects
fl_dsgn3='null'       !design matrix for social interaction (continous)
...
wmj=0        !default: no SNP BLUP component
...
sclf=-1      !scale factor for snp_blup2
rtmx2='null'    !relationship matrix scale factor
rtmx3='null'    !accounting for missingness difference between pops
wt_par='null'    !weighting factor parameters
prs='null'    !PRS anlaysis
targ='null'    !Target plink file for PRS anlaysis
wt_res='null'    !weighting factor for residual structure
hrtmx='null'    !H matrix 
nrm='null'    !H matrix 
grmout='null'    !to output modified grm  
method=1    !H matrix method, 1 or 2
tune=0
blend=1.0
sbv='null'   !snp_blup option, a or b
vgpy='null'
sbv2='null'   !snp_blup2 using Ginv
ginv='null'   !snp_blup2 using Ginv
delta='null'
delta2='null'
simcoal='null'
simcoal2='null'
...
do i=1, (narg/2)
  ...
  if (filnam=='-check') then     !number of traits ***
    call get_command_argument(i*2, filnam)
      print*,'check     : ',trim(filnam)
      read(filnam,*)check
  end if
  ...
  if (filnam=='-bv') then     !EBV only ***
    call get_command_argument(i*2, filnam)
      fl11=filnam
      print*,'ebv output: ',trim(fl11)
  end if
  if (filnam=='-bvr') then     !EBV with r2 ***
    call get_command_argument(i*2, filnam)
      fl11r=filnam
      print*,'ebv output: ',trim(fl11r)
  end if
  if (filnam=='-pm') then     !P matrix output  ***
    call get_command_argument(i*2, filnam)
      fl11rr=filnam
      print*,'P to be in: ',trim(fl11rr)
  end if
  if (filnam=='-gpg') then     !GPG matrix output  ***
    call get_command_argument(i*2, filnam)
      fl11rrr=filnam
      print*,'GPG in    : ',trim(fl11rrr)
  end if
  if (filnam=='-submat') then     !sub matrix output  ***
    call get_command_argument(i*2, filnam)
      subgrm=filnam
      print*,'submat in : ',trim(subgrm)
  end if
  if (filnam=='-snpbv') then     !SNP EBV ***
    call get_command_argument(i*2, filnam)
      fl111=filnam
      print*,'W matrix  : ',trim(fl111)
  end if
  if (filnam=='-snpbvr') then     !SNP EBV with r2 ***
    call get_command_argument(i*2, filnam)
      fl111r=filnam
      print*,'W matrix  : ',trim(fl111r)
  end if
  if (filnam=='-snpblup') then     !number of traits ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)wmj
      print*,'snpblup # : ',wmj
  end if
  !if (filnam=='-wmatcol') then     !number of traits ***
  !  call get_command_argument(i*2, filnam)
  !    read(filnam,*)wmi
  !    print*,'W col #   : ',wmi
  !end if

  if (filnam=='-covgm') then     !number of traits ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)cov_idx
      print*,'cov (g,m) : ',cov_idx
  end if
  ...
  if (filnam=='-rxe') then     !RxE estimation without GRM ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)rxe
      print*,'rxe       : ',rxe
  end if
  ...
  if (filnam=='-dsgn') then     !spline random regression ***
    call get_command_argument(i*2, filnam)
      fl_dsgn=filnam
      print*,'dsgn file : ',trim(fl_dsgn)
  end if
  if (filnam=='-dsgn2') then     !spline random regression ***
    call get_command_argument(i*2, filnam)
      fl_dsgn2=filnam
      print*,'dsgn2 file: ',trim(fl_dsgn2)
  end if
  if (filnam=='-dsgn3') then     !spline random regression ***
    call get_command_argument(i*2, filnam)
      fl_dsgn3=filnam
      print*,'dsgn3 file: ',trim(fl_dsgn3)
  end if
  ...
  if (filnam=='-sclf') then     !scale parameter for grm ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)sclf
      print*,'sclf      : ',trim(filnam)
  end if
  if (filnam=='-rtmx2') then     !scale parameter for grm ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)rtmx2
      print*,'rtmx2     : ',trim(filnam)
  end if
  if (filnam=='-rtmx3') then     !scale parameter for grm ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)rtmx3
      print*,'rtmx3     : ',trim(filnam)
  end if
  if (filnam=='-prs') then     !scale parameter for grm ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)prs
      print*,'prs       : ',trim(filnam)
  end if
  if (filnam=='-targ') then     !scale parameter for grm ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)targ
      print*,'targ      : ',trim(filnam)
  end if
  ...
  if (filnam=='-nrm') then     !ped info for h matrix (ssgblup) ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)nrm
      print*,'nrm      : ',trim(filnam)
  end if
  if (filnam=='-tune') then     !snp_blup option ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)tune
      print*,'tune      : ',trim(filnam)
  end if
  if (filnam=='-blend') then     !snp_blup option ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)blend
      print*,'blend     : ',trim(filnam)
  end if
  if (filnam=='-method') then     !snp_blup option ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)method
      print*,'method    : ',trim(filnam)
  end if
  if (filnam=='-grmout') then     !ped info for h matrix (ssgblup) ***
    call get_command_argument(i*2, filnam)
      read(filnam,*)grmout
      print*,'grmout    : ',trim(filnam)
  end if
  ...
  if (filnam=='-sbv2') then     !snp_blup2 with ebv file
    call get_command_argument(i*2, sbv2)
      print*,'snp_blup2 : ',trim(sbv2)
  end if
  if (filnam=='-ginv') then     !snp_blup2 with Ginv file
    call get_command_argument(i*2, ginv)
      print*,'ginv      : ',trim(ginv)
  end if
  ...
  if (filnam=='-pql') then     !PQL
    call get_command_argument(i*2, filnam)
      read(filnam,*)pql
      print*,'PQL       : ',trim(filnam)
  end if

  if (filnam=='-mtpreg') then     !number of traits ***
    call get_command_argument(i*2, filnam)
      print*,'mtpreg    : ',trim(filnam)
      read(filnam,*)mtpreg
  end if
...
if (rtmx2.ne.'null' .and. plink.eq.'null') then
  print*,'with -rtmx2, -plink should be specified'
  pause
end if
if (rtmx3.ne.'null' .and. plink.eq.'null') then
  print*,'with -rtmx3, -plink should be specified'
  pause
end if
...
if (sbv2.ne.'null' .and. ginv.eq.'null') then
  print*,'with -sbv2, -ginv file should be specified'
  pause
end if

if (wmj.ne.0 .and. fl111.eq.'null' .and. fl111r.eq.'null') then
  print*,'with -snpblup, -snpbv or -snpbvr file (W matrix) should be specified'
  pause
end if
...
!if (fl_dsgn.ne.'null') then
if (fl_dsgn.ne.'null'.or.fl_dsgn2.ne.'null'.or.fl_dsgn3.ne.'null') then
  if (cov_idx==0) mn=2   !g,m
  if (cov_idx==1) mn=3   !g,m,cov
  if (cov_idx==2) mn=4   !g,m,cov,p
end if
...
allocate(ccm(n,10000),qcm(n,10000),tmp_qcm(10000),ccdet(10000),qcdet(10000))
allocate(cdum_t(tn),cc_lvl(tn))
!allocate(xmm(tn,n,1000),fnm(tn),rnm(tn),ccdet2(tn),ccdet3(tn))
allocate(xmm(tn,n,10000),fnm(tn),rnm(tn),ccdet2(tn),ccdet3(tn))
...
11  continue
  ...
  do i=1,rn               !order same as in .dat
    if (rn.ne.n) then
      print*,'-rxe with the same # and order IDs in fam and dat >>> check'
      pause
    end if
    idx_obs(i,2)=i        !order same as in .dat
    idx_y(i,:)=y(i,:)
  end do
  end if
...
!print*,'data checked:',real(t2_cpu-t1_cpu)
print*,'data check >>> take 1:',real(t2_cpu-t1_cpu)
...
if (cc==1) then
  ...
  !print*,'cc_c:',cc_c
  if (cc_c>10000) then
    print*,'no. column is over 10000 >>> check',cc_c
  end if
...
if (qc==1) then
  ...
  !print*,'qc_c:',qc_c
  if (qc_c>10000) then
    print*,'no. column is over 10000 >>> check',qc_c
  end if
...
allocate(cc_zi(tn,cc_c),ccn(tn,cc_c,10000))
...
60    continue
  ...
  call cpu_time (t2_cpu)
  !print*,'fixed effects sorted out:',real(t2_cpu-t1_cpu)
  print*,'data check >>> take 2:',real(t2_cpu-t1_cpu)
...
!!elseif (fl_dsgn.ne."null") then    !design matrix given
!elseif (fl_dsgn.ne."null" .or. fl_dsgn2.ne."null") then    !design matrix given
...
elseif (tmg==1) then
  do zi=1,mn
    ...
    elseif (tzb(zi)=="bin" .or. tzb(zi)=="binary" .or. tzb(zi)=="b") then
      ...
      !open (unit=71,file=grm_known(zi),form='unformatted',access='stream')
      print*,'grm file:',grm_known(zi) !,mn
      k=0
      do i=1,n
        do j=1,i
  ...
  if (bmg==-1 .and. zmg==-1) then
    do zi=1,mn
      if ((fl_dsgn.ne."null".or.fl_dsgn2.ne."null".or.fl_dsgn3.ne.'null') .and. zi>1) exit    !design matrix given
      open (unit=71,file=grm_known(zi),status='old')
      print*,'grm file:',grm_known(zi)
      do
        read (71,*,iostat=io)i,j,x1   !order same as in .fam (check in RTMX)
...
if (rtmx2.ne.'null') then
  call rtmx_2 (plink,rtmx2,fl5)
  goto 1100
end if
if (rtmx3.ne.'null') then
  call rtmx_3 (plink,rtmx3,fl5)
  goto 1100
end if
...
if (hrtmx.ne.'null') then
  if (nrm.ne.'null') then
    call hrtmx_2 (hrtmx,nrm,GRM,pedn,n,mn,pedor,tune,blend,method,fl5,plink,grmout)
    goto 1100
  end if

  !print*,pedn,n,mn
  !call hrtmx_ (hrtmx,GRM,pedn,n,mn,pedor,tune,blend,fl5)
  !call hrtmx_ (hrtmx,GRM,pedn,n,mn,pedor,tune,blend,fl5,plink)
  !call hrtmx_ (hrtmx,GRM,pedn,n,mn,pedor,tune,blend,method,fl5,plink)
  call hrtmx_ (hrtmx,GRM,pedn,n,mn,pedor,tune,blend,method,fl5,plink,grmout)
  goto 1100
end if

!prs analysis *******************************************************
if (prs.ne.'null') then
  call prs_acc (plink,prs,targ,fl4,fl5)
  goto 1100
end if
...
!snp_blup **********************************************************
if (sbv.ne.'null') then
  call sblup(plink,vgpy,sbv,fl11rr,fl4,fl5,sclf)   !snp_blup
  goto 1100
end if

!snp_blup2 **********************************************************
if (sbv2.ne.'null') then
  call sblup2(plink,ginv,sbv2,fl11rr,fl11rrr,fl4,fl5,sclf)   !snp_blup
  goto 1100
end if

!snp_blup2 and BLUP GWAS ********************************************
if (wmj .ne. 0) then
  !print*,fl111,fl111r,wmj,fl11,fl11r,rn,rnm,trn,n,tn,fnm,tfn,mn,nit,conv,fl4,fl5,fl24
  if (icove==0) then
    call aireml_m_h_sblup (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,wmj,fl111,fl111r)
  else
    call aireml_m_h2_sblup (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,wmj,fl111,fl111r)
  end if
  goto 1100
end if 

!sub matrix extraction ************************************************
if (subgrm.ne.'null') then
  call submat (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,subgrm)
  goto 1100
end if
...
if (fl_dsgn.ne."null") then
  if (icove==0) then
    call aireml_m_h_dsgn (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,fl_dsgn,cov_idx)
  else
    call aireml_m_h2_dsgn (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,fl_dsgn,cov_idx)
  end if
  goto 1100
end if
!design matrix for social interaction *************************************
if (fl_dsgn2.ne."null") then
  if (icove==0) then
    call aireml_m_h_dsgn2 (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,fl_dsgn2,cov_idx)
  else
    call aireml_m_h2_dsgn2 (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,fl_dsgn2,cov_idx)
  end if
  goto 1100
end if
!design matrix for social interaction for continous variable**************
if (fl_dsgn3.ne."null") then
  if (icove==0) then
    call aireml_m_h_dsgn3 (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,fl_dsgn3,cov_idx)
  else
    call aireml_m_h2_dsgn3 (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res,fl_dsgn3,cov_idx)
  end if
  goto 1100
end if
...
!multiple regression for SNP-h2
if (mtpreg==1) then
  call h2_mtpreg(plink,idx_y,n,fl5)
  goto 1100
end if
...
if (rxe==1) then
  mn=1
  if (icove==0) then
    if (fl_rnm.ne."null") then
      if (mrnm==0) then   !uni rnm
        print*,'-rxe should be with -mrnm >>> check'
        pause
      elseif (mrnm>=1) then  !multi rnm
        call aireml_rxe (yidx,idx_y,rn,rnm,trn,n,mrnm,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl_rnm,fl24,xmm,nit,conv,thn)
        goto 1100
      end if
    end if

  elseif (icove==1) then
    if (fl_rnm.ne."null") then   !rnm
      if (mrnm==0) then     !uni rrm
        print*,'rxe should be with -mrnm >>> check'
        pause
      elseif (mrnm>=1) then  !multi rrm
        !call aireml_rxe2 (yidx,idx_y,rn,rnm,trn,n,mrnm,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl_rnm,fl24,xmm,nit,conv,thn)
      end if
    end if
  end if

elseif (rxe==2) then   !ML
  mn=1
  if (icove==0) then
    if (fl_rnm.ne."null") then
      if (mrnm==0) then   !uni rnm
        ...
        pause
      elseif (mrnm>=1) then  !multi rnm
        call aireml_rxe_ML (yidx,idx_y,rn,rnm,trn,n,mrnm,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl_rnm,fl24,xmm,nit,conv,thn)
        goto 1100
      end if
    end if
  end if

elseif (rxe==3) then   !ML NR
  mn=1
  if (icove==0) then
    if (fl_rnm.ne."null") then
      if (mrnm==0) then   !uni rnm
        ...
        pause
      elseif (mrnm>=1) then  !multi rnm
        call aireml_rxe_ML_NR (yidx,idx_y,rn,rnm,trn,n,mrnm,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl_rnm,fl24,xmm,nit,conv,thn)
        goto 1100
      end if
    end if
  end if
      ...
      if (fl12.ne."null") then    !eig
        ...
        elseif (fl_mcv.ne."null") then
          ...
          if (gwas.ne.0) then
            ...
            if (pql.ne.0) then
              call aireml_m_h_pql (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res)
            else
              !call aireml_m_h (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res)
              call aireml_m_h (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl11rr,fl11rrr,fl24,xmm,nit,conv,wt_res)
            end if
          end if
        end if
      end if
    ...
    if (irgt.ne.-9.0D0) then
      ...
      if (fl12.ne."null") then    !eig
        ...
        elseif (fl_mcv.ne."null") then   !rnm
          ...
          !print*,tfn,mn,tn,trn,rnm,rn
          !call aireml_m_h2 (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl24,xmm,nit,conv,wt_res)
          call aireml_m_h2 (GRM,yidx,idx_y,rn,rnm,trn,n,tn,fnm,tfn,mn,fl4,fl5,fl11,fl11r,fl11rr,fl11rrr,fl24,xmm,nit,conv,wt_res)
        end if
      end if
    end if
