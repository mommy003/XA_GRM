...
subroutine rtmx_2 (filnam,wt_par,f41)
...
integer,allocatable::wt_idx1(:,:) !,wt_idx2(:)
...
do zi=1,npop
  ...
  do zj=1,(zi-1)
    ...
    !call dgemm ('N','T',k1,k2,nm,1/sqrt(diag1*diag2),kvm,k1,kvm2,k2,0.0D0,rtmx_off,k1)
    !call dgemm ('N','T',k1(zi),k1(zj),nm,1/sqrt(diags),kvm,k1(zi),kvm2,k1(zj),0.0D0,rtmx_off,k1(zi))
  ...
  do im=1,nm
    if (htz(im,zi)>0.and.htz(im,zi)<1.and.htz(im,zj)>0.and.htz(im,zj)<1) then  !check, exclude NaN htz
      ...
      htz2b=htz2b+((htz(im,zj)**(1+sclf(zj)))/k1(zj)) 
    end if
    !print*,htz1(zi)
  end do
  !rtmx_off=rtmx_off+sqrt(htz1(zi)*htz1(zj))
  rtmx_off=rtmx_off+sqrt(htz2a*htz2b)
  !rtmx_off=rtmx_off+htz2
  !print*,rtmx_off !htz2,sqrt(htz1(zi)*htz1(zj))
